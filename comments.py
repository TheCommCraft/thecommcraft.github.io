...
def load_comments():
  ...
  for comment in comments_root:
    ...
    comments.extend([f"   {i['author']['username']}: @{comment['author']['username']} {i['content']}" for i in get("https://api.scratch.mit.edu/users/TheCommCraft/projects/847888429/comments/{comment[id]}/replies?limit=10").json()])
    if len(comments) > 10:
        break
