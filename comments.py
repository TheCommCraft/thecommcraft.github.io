...
def load_comments():
  ...
  for comment in comments_root:
    ...
    comments.extend([(f"        {i['author']['username']}: @{users[i['commentee_id']]} {i['content']}", users.__setitem__(i['author']['id'], i['author']['username']))[0] for i in get(f"https://api.scratch.mit.edu/users/TheCommCraft/projects/847888429/comments/{comment['id']}/replies?limit=17").json()])
    if len(comments) >= 18:
        break
